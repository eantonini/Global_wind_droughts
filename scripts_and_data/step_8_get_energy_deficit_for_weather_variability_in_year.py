...
Description:
    ...
    This script calculates the energy deficit for the weather variability in a given year.
'''
...
if settings.use_dask_mpi:
    ...
    dask_workers_folder_name = 'dask_workers_step_8_'+str(year)
    client = utilities.create_dask_cluster_and_client(dask_workers_folder_name)

# Define the log file name
log_file_name = 'step_8_'+str(year)
...
# Calculate the weather variability
energy_deficit_for_weather_variability = utilities.calc_energy_deficit(wind_power_density_time_series, wind_power_density_time_series, 'climatological_target', year, climatological_time_series=climatological_wind_power_density_time_series)
utilities.save(settings.result_directory+'/{:d}-energy_deficit_for_weather_variability.nc'.format(year), energy_deficit_for_weather_variability)   
utilities.write_to_log_file(log_file_name, 'Weather variability calculated\n\n')
